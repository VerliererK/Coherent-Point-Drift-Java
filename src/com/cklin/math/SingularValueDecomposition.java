...
public class SingularValueDecomposition {
     ...
     */
    private final double[][] cachedV;
    /**
     * Cached value of transposed V matrix.
     */
    private double[][] cachedVt;
    /**
     ...
     */
    public SingularValueDecomposition(final double[][] matrix) {
        final double[][] A;

        // "m" is always the largest dimension.
        if (matrix.length < matrix[0].length) {
            transposed = true;
            A = ArraysUtils.transpose(matrix);
            m = matrix[0].length;
            n = matrix.length;
        } else {
            transposed = false;
            A = ArraysUtils.copy(matrix);
            m = matrix.length;
            n = matrix[0].length;
        }
    ...
    public double[][] getU() {
        ...
        return ArraysUtils.copy(cachedU);
    ...
    public double[][] getUT() {
        if (cachedUt == null) {
            cachedUt = ArraysUtils.transpose(getU());
        }
        // return the cached matrix
        return cachedUt;
    }
    ...
    public double[][] getS() {
        if (cachedS == null) {
            ...
            cachedS = ArraysUtils.diag(singularValues);
        }
        return cachedS;
    }
    ...
    public double[][] getV() {
        // return the cached matrix
        return ArraysUtils.copy(cachedV);
    }
    ...
    public double[][] getVT() {
        if (cachedVt == null) {
            cachedVt = ArraysUtils.transpose(getV());
        }
        // return the cached matrix
        return cachedVt;
    }
     ...
     */
    public double[][] getCovariance(final double minSingularValue) {
        ...
        if (dimension == 0) {
            throw new IllegalArgumentException("TOO_LARGE_CUTOFF_SINGULAR_VALUE");
        ...
        final double[][] jv = getVT();
        for (int i = 0; i < jv.length; i++) {
            for (int j = 0; j < jv[0].length; j++)
                jv[i][j] /= singularValues[i];
        }

        return ArraysMath.mmul(ArraysUtils.transpose(jv), jv);
    }
    ...
    public boolean isNonSingular() {
        return getSolver().isNonSingular();
    }

    public double[][] getInverse() {
        return getSolver().getInverse();
    }
    ...
    private Solver getSolver() {
        if (solver == null) {
            solver = new Solver(singularValues, getUT(), getV(), getRank() == m, tol);
        }
        // return the solver
        return solver;
    }
    ...
    private static class Solver {
         ...
         */
        private final double[][] pseudoInverse;
        /**
         ...
         */
        private Solver(final double[] singularValues, final double[][] uT,
                       final double[][] v, final boolean nonSingular, final double tol) {
            final double[][] suT = ArraysUtils.copy(uT);
            for (int i = 0; i < singularValues.length; ++i) {
                final double a;
                if (singularValues[i] > tol) {
            ...
            pseudoInverse = ArraysMath.mmul(v, suT);
        ...
        public double[] solve(final double[] b) {
            final int nRows = pseudoInverse.length;
            final int nCols = pseudoInverse[0].length;
            if (b.length != nCols) {
                throw new IllegalArgumentException("DimensionMismatch");
            }
            final double[] out = new double[nRows];
            for (int row = 0; row < nRows; row++) {
                final double[] dataRow = pseudoInverse[row];
                double sum = 0;
         ...
         */
        public double[][] solve(final double[][] b) {
            return ArraysMath.mmul(pseudoInverse, b);
        }
         ...
         */
        public double[][] getInverse() {
            return ArraysUtils.copy(pseudoInverse);
        }
    }
}
